...
public:
    //==================Assignment===================//
    MatrixX<T>(uint16_t m=0, uint16_t n=0, T* data=0);// constructor
    ...
    inline T& set(uint16_t i, uint16_t j); // sets one single element
    inline T& operator()(uint16_t i, uint16_t j=0); //sets and returns one single element
    ...
    //==================Matrix Data==================//
    inline T& get(uint16_t i, uint16_t j) const; //retrieves one single element
    T trace() const; // retrieves the trace
    inline uint16_t rows() const; // retrieves the number of rows
    inline uint16_t columns() const; // retrieves the number of columns
    inline uint16_t length() const; // retrieves the lenght of the Matrix
    ...
    MatrixX<T> subMatrix(uint16_t row_top, uint16_t col_left, uint16_t row_bottom, uint16_t col_right) const; //returns a subMatrix of the original
    //===============Auxiliary Functions==============//
    template <typename T2> inline MatrixX<T>& copyData(const T2* data); //sets new data
    inline T& getData(uint16_t i) const; //retrieve a single data
...
private:
    ...
    inline uint16_t index(uint16_t i, uint16_t j) const;
    //-------Private Variables-----//
    T* _data;
    uint16_t _nrows;
    uint16_t _ncols;
...
typedef MatrixX<uint16_t>                MatrixXus;
