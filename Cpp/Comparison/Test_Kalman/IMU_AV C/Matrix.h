...
//==================Assignment===================//
Matrix newMatrix(uint8_t rows, uint8_t cols, float *data); //creates a new matrix containing a pointer to an external array
...
uint8_t matMult_lhsT(Matrix lhs, Matrix rhs, Matrix result); // Matrix multiplication with lhs transposed------//
uint8_t matMult_rhsT(Matrix lhs, Matrix rhs, Matrix result); // Matrix multiplication with rhs transposed------//
