...
//-----------------------Constructor------------------------//
template<typename T>
MatrixX<T>::MatrixX(uint16_t m, uint16_t n, T* data) {
    this->_nrows = m;
    this->_ncols = n;
    this->_data = 0;
    allocate();
...
//--------------------Sets single element-------------------//
template<typename T>
inline T& MatrixX<T>::set(uint16_t i, uint16_t j) {
    return _data[index(i,j)];
}

//--------------Sets and returns single element-------------//
template<typename T>
inline T& MatrixX<T>::operator()(uint16_t i, uint16_t j){
    return _data[index(i,j)];
}
...
MatrixX<T>& MatrixX<T>::identity() {
    for(uint16_t i=0; i<_nrows*_ncols; i++) {
...
template <typename T2> MatrixX<T> MatrixX<T>::operator*(const MatrixX<T2> &rhs) const {
    ...
    for (uint16_t i=0; i < _nrows; i++)
        for (uint16_t j=0; j < rhs.columns(); j++)
            for (uint16_t k=0; k < _ncols; k++)
...
MatrixX<T> MatrixX<T>::operator~() const {
    ...
    for (uint16_t ii=0;ii<_nrows;ii++){
        for (uint16_t jj=0;jj<_ncols;jj++){
...
template<typename T>
inline T& MatrixX<T>::get(uint16_t i, uint16_t j) const{
    return _data[index(i,j)];
}
...
T MatrixX<T>::trace() const {
    T result = 0.0;
    for (uint16_t i=0; i<_nrows && i<_ncols; i++) {
        result += get(i, i);
    }
    return result;
}
...
template<typename T>
inline uint16_t MatrixX<T>::rows() const{
    return _nrows;
}

//-------Returns the number of columns-------//
template<typename T>
inline uint16_t MatrixX<T>::columns() const{
    return _ncols;
}

//------------Returns the length------------//
template<typename T>
inline uint16_t MatrixX<T>::length() const{
...
//------------Returns a subMatrix------------//
template<typename T>
MatrixX<T> MatrixX<T>::subMatrix(uint16_t row_top, uint16_t col_left, uint16_t row_bottom, uint16_t col_right) const {
    uint16_t rows = row_bottom-row_top+1;
    uint16_t cols = col_right-col_left+1;
...
template<typename T>
inline T& MatrixX<T>::getData(uint16_t i) const{
    return _data[i];
}
...
template<typename T>
inline uint16_t MatrixX<T>::index(uint16_t i, uint16_t j) const{
    return (i * _ncols + j);
}
