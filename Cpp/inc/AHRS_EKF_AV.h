...
#define AHRS_EKF_AV_C_DAMP0                                                                                            \
    0.07413f //initial value of the damping coefficient, inversely proportional to the mass of the quadrotor, in N*s/m
#define AHRS_EKF_AV_INCL0                                                                                              \
    -1.0734f //initial value of the inclination of the magnetic field, in rad (Northern Emisphere positive (pointing down), Southern Emisphere negative (pointing up))
...
class AHRS_EKF_AV {
  ...
  public:
    ...
    MatrixXf
        u; // state vector (Roll=Phi, Pitch=Theta, Yaw=Psi, Xd, Yd, Zd, c_damp, b_az, incl, b_gx, b_gy, b_gz) angles in rad, angular velocities in rad/s, velocities in m/s, c_damp in N*s/m
    ...
    void set_starting_values(float Phi_0 = AHRS_EKF_AV_PHI0, float Theta_0 = AHRS_EKF_AV_THETA0,
                             float Psi_0 = AHRS_EKF_AV_PSI0, float c_damp_0 = AHRS_EKF_AV_C_DAMP0,
                             float incl_0 = AHRS_EKF_AV_INCL0);
    void set_input_noises(float g_xy = AHRS_EKF_AV_GXY_NOISE, float g_z = AHRS_EKF_AV_GZ_NOISE,
                          float a_z = AHRS_EKF_AV_AZ_NOISE, float c_damp = AHRS_EKF_AV_C_DAMP_NOISE,
                          float b_az = AHRS_EKF_AV_B_AZ_NOISE, float incl = AHRS_EKF_AV_INCL_NOISE,
                          float b_g = AHRS_EKF_AV_B_G_NOISE);
    void set_acc_noises(float a = AHRS_EKF_AV_A_NOISE);
    void set_mag_noises(float m = AHRS_EKF_AV_M_NOISE);

    void set_vel_xy_noise(float v_xy = AHRS_EKF_AV_VXY_NOISE) {
        _r_vxy = v_xy;
        return;
    };

    void set_vel_z_noise(float v_z = AHRS_EKF_AV_VZ_NOISE) {
        _r_vz = v_z;
        return;
    };

    void set_vel_ne_noise(float v_ne = AHRS_EKF_AV_VNE_NOISE) {
        _r_vne = v_ne;
        return;
    };

    void set_vel_d_noise(float v_d = AHRS_EKF_AV_VD_NOISE) {
        _r_vd = v_d;
        return;
    };
